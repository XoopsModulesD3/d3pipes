...
function d3pipes_common_get_pipe4assign( $mydirname , $pipe_id )
{
	...
	$pipe4assign = array(
		...
		'id' => intval( $pipe_id ) ,
		'name' => $myts->makeTboxData4Show( $pipe_row['name'] ) ,
		'name4xml' => htmlspecialchars( $pipe_row['name'] , ENT_QUOTES , _CHARSET ) ,
		'url' => $myts->makeTboxData4Show( $pipe_row['url'] ) ,
		'url4xml' => htmlspecialchars( $pipe_row['url'] , ENT_QUOTES , _CHARSET ) ,
		'image' => $myts->makeTboxData4Show( $pipe_row['image'] ) ,
		'image4xml' => htmlspecialchars( $pipe_row['image'] , ENT_QUOTES , _CHARSET ) ,
		'description' => $myts->displayTarea( $pipe_row['description'] ) ,
		'description4xml' => htmlspecialchars( $myts->displayTarea( $pipe_row['description'] ) , ENT_QUOTES , _CHARSET ) ,
